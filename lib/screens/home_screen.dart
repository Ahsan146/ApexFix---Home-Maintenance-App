    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          if (customer != null) Padding(padding: const EdgeInsets.only(right: 16), child: CircleAvatar(radius: 17, backgroundColor: const Color(0xFF5146D8), child: Text(customer.fullName.isNotEmpty ? customer.fullName[0].toUpperCase() : 'U', style: const TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.w800)))),
        ],
      ),
      body: SingleChildScrollView(
        padding: const EdgeInsets.fromLTRB(16, 18, 16, 28),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text('GOOD TO SEE YOU', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w800, letterSpacing: 1.6, color: primary)),
          ...
          Container(padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 13), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(12), border: Border.all(color: line)), child: const Row(children: [Icon(Icons.search_rounded, color: muted, size: 20), SizedBox(width: 10), Text('Search a service, repair or technician...', style: TextStyle(color: muted, fontSize: 12)), Spacer(), Icon(Icons.tune_rounded, color: muted, size: 17)])),
          const SizedBox(height: 16),
          if (active != null && active.status != BookingStatus.completed && active.status != BookingStatus.cancelled)
          ...
          _sectionTitle('Quick dispatch', 'Emergency help'),
          const SizedBox(height: 10),
          Container(padding: const EdgeInsets.all(14), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(15), border: Border.all(color: const Color(0xFFFFD9B5))), child: Row(children: [Container(width: 42, height: 42, decoration: BoxDecoration(color: const Color(0xFFFFA51F), borderRadius: BorderRadius.circular(11)), child: const Icon(Icons.flash_on_rounded, color: Colors.white)), const SizedBox(width: 12), const Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('24/7 Rapid Emergency Dispatch', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w800)), SizedBox(height: 3), Text('Short circuit, severe leak or urgent breakdown', style: TextStyle(fontSize: 11, color: muted))])), const Icon(Icons.arrow_forward_rounded, color: primary, size: 19)])),
          const SizedBox(height: 22),
          _sectionTitle('Services', 'View all'),
          const SizedBox(height: 11),
          GridView.builder(shrinkWrap: true, physics: const NeverScrollableScrollPhysics(), itemCount: MockData.categories.length, gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 3, crossAxisSpacing: 10, mainAxisSpacing: 10, childAspectRatio: .91), itemBuilder: (context, i) {
